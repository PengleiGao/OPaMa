...
from .modules import get_cartesian, CLIPVisionEmbeddings, CrossAttnStoreProcessor, FeedForward
import numpy as np
...
from .my_modules import CusMamba,MambaAdapter
import PIL
import pl_bolts
...
class InpaintMamba(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.cache_dir = '/home/gaop/cache_models'
    ...
    def init_models(self, sd_model_id, clip_model_id, transformer_config, controlnet_id=None):
        ...
        if self.use_adapter:
            ...
            if self.disable_geo:
                inchannel = 3
            self.adapter = MambaAdapter(in_channels=inchannel)
            self.adapter.requires_grad_(True)
            self.net_to_save['adapter'] = self.adapter

        if not self.disable_ar_cond:

            self.projector = CusMamba(dim=clip_dim)
            self.text_embedding = torch.nn.Embedding(77, self.text_encoder.config.hidden_size)
            self.vision_embedding = torch.nn.Embedding(7, self.text_encoder.config.hidden_size)
    ...
    '''
    def get_cond_embed(self, pixel_cube, image, caption):
        ...
        cube_feat = rearrange(cube_feat, '(b p) c -> b p c', b=bs, p=n_patch)#torch.Size([1, 6, 1024])

        # the nfov image
        input_feat = self.clip_encode_image(image, False, True, False)
        input_feat = rearrange(input_feat, 'b c -> b 1 c')#torch.Size([1, 1, 1024])

        # text 
        text_feat = self.clip_encode_prompt(caption)#torch.Size([1, 77, 1024])
        ...
        img_feat = torch.cat([input_feat,cube_feat],dim=1)

        all_feat = torch.cat([img_feat, text_feat], dim=1)
        embedding = torch.cat([vision_embedding, text_embedding], dim=1)

        input_feat = all_feat+embedding
        global_feat, gate_fate = self.projector(input_feat)

        #cond_embed = torch.cat([cond_embed,text_feat],dim=1)
        alpha = gate_fate.sigmoid()
        cond_embed = global_feat * alpha + (1-alpha)*all_feat

        return cond_embed

    def get_uncond_embed(self, bs):
        uncond_embed = self.uncond_embed(torch.LongTensor(list(range(7))).to(self.device))
    ...
    def get_adapter_feat(self, pixel_cube, pos_cube, image, pos):
        ...
        pers = pers.unsqueeze(1)

        img_all=torch.cat([cube, pers], dim=1)

        adapter_feat = self.adapter(img_all)
    ...
    def inpaint(self,
        ...
        for i, t in enumerate(timesteps):
            ...
            if guidance_scale>0:
                ...
                noise_pred = self.unet_pred(latent_model_input, t, uncond_prompt_embeds, adapter_feat)
    ...
    def forward(self, batch):
        ...
        if random.uniform(0, 1) < 0.8: # puncond = 0.9/[0.8]/0.5 according to https://arxiv.org/pdf/2207.12598.pdf

            if random.uniform(0, 1) < 0.5:
                caption = ["" for _ in caption]
    ...
    def configure_optimizers(self):
        ...
        #lr_scheduler = pl_bolts.optimizers.lr_scheduler.LinearWarmupCosineAnnealingLR(opt, warmup_epochs=10, max_epochs=240,eta_min=1e-6)
        return opt
    #pip install pytorch-lightning-bolts  pl_bolts.optimizers.lr_scheduler.LinearWarmupCosineAnnealingLR(optimizer2)
