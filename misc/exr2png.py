...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_dir", type=str, default='/mnt/e/BaiduNetdiskDownload/IndoorHDRDatasetReexposedNoRedDotsNoInpaintingOct18/IndoorHDRDataset2018', required=False)
    parser.add_argument("--out_dir", type=str, default='/mnt/e/Datasets/indoorPanosPng', required=False)
    parser.add_argument("--num_workers", type=int, default=20, required=False)
    ...
    nfs=[]
    for f in fs:
        if f.replace('.exr', '.png') not in dfs:
            nfs.append(f)

    tasks = (delayed(process_exr)(os.path.join(source, exr), os.path.join(dest, exr.replace('.exr', '.png'))) for exr in nfs)
    executor(tasks)
