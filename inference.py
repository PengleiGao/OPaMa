...
from diffusers import PNDMScheduler,DPMSolverMultistepScheduler
...
#model.unet.enable_xformers_memory_efficient_attention(attention_op=MemoryEfficientAttentionFlashAttentionOp)
...
scheduler = DPMSolverMultistepScheduler.from_pretrained(model.sd_model_id, subfolder='scheduler', cache_dir=model.cache_dir)
...
def get_input(target_yaw, target_pitch, gen_equi_local):
    ...
    return pixel_cube, pos_cube, input_image, mask, target, pos
...
def inpaint_all():
    ...
    for i in track(range(len(ds))):
        ...
        # print(source_image_local.size(), source_image_local.min(),source_image_local.max()) #torch.Size([3, 512, 512]) tensor(-0.9137) tensor(1.)
        ...
        for jdx, (yaw, pitch) in tqdm(enumerate(gen_coord_list)):
            ...
            pixel_cube, pos_cube, input_image, mask, target, pos = get_input(yaw, pitch, gen_equi_local)
            caption = ds.get_caption(i)
